...
WITH previous_orders as 
    ...
    current_orders as 
    (SELECT DATE_TRUNC('day',order_created_date_est::date) as date, 'day' as date_granularity, COUNT(*) as utm_acquisitions, 0 as utm_first_orders, utm_campaign,utm_content, utm_medium, utm_source
    FROM {{ source('gsheet_raw','utm_acquisitions_2024') }}
    GROUP BY 1,2,5,6,7,8
    
    UNION ALL
    
    SELECT DATE_TRUNC('week',order_created_date_est::date) as date, 'week' as date_granularity, COUNT(*) as utm_acquisitions, 0 as utm_first_orders, utm_campaign,utm_content, utm_medium, utm_source
    FROM {{ source('gsheet_raw','utm_acquisitions_2024') }}
    GROUP BY 1,2,5,6,7,8
    
    UNION ALL
    
    SELECT DATE_TRUNC('month',order_created_date_est::date) as date, 'month' as date_granularity, COUNT(*) as utm_acquisitions, 0 as utm_first_orders, utm_campaign,utm_content, utm_medium, utm_source
    FROM {{ source('gsheet_raw','utm_acquisitions_2024') }}
    GROUP BY 1,2,5,6,7,8
    
    UNION ALL
    
    SELECT DATE_TRUNC('quarter',order_created_date_est::date) as date, 'quarter' as date_granularity, COUNT(*) as utm_acquisitions, 0 as utm_first_orders, utm_campaign,utm_content, utm_medium, utm_source
    FROM {{ source('gsheet_raw','utm_acquisitions_2024') }}
    GROUP BY 1,2,5,6,7,8
    
    UNION ALL
    
    SELECT DATE_TRUNC('year',order_created_date_est::date) as date, 'year' as date_granularity, COUNT(*) as utm_acquisitions, 0 as utm_first_orders, utm_campaign, utm_content, utm_medium, utm_source
    FROM {{ source('gsheet_raw','utm_acquisitions_2024') }}
    GROUP BY 1,2,5,6,7,8)
